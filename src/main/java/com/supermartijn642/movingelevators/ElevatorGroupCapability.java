...
public class ElevatorGroupCapability {
    ...
    public static void onJoinWorld(PlayerEvent.PlayerChangedDimensionEvent e){
        ...
        if(groups != null)
            MovingElevators.channel.sendTo(new ElevatorGroupsPacket(groups.write()), player);
    }
    ...
    public void updateGroup(ElevatorGroup group){
        if(!this.world.isRemote && group != null)
            MovingElevators.channel.sendToDimension(new ElevatorGroupPacket(this.writeGroup(group)), this.world.provider.getDimension());
    }

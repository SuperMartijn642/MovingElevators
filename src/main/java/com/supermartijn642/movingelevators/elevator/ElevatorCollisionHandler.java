...
public class ElevatorCollisionHandler {
    ...
    public static void handleEntityCollisions(World level, AxisAlignedBB bounds, List<AxisAlignedBB> boundingBoxes, Vector3d position, Vector3d motion){
    ...
    private static void handleHorizontalCollision(Entity entity, AxisAlignedBB box){
        ...
        Vector3d oldEntityPos = new Vector3d(entity.xo, entity.yo, entity.zo);
        Vector3d entityMotion = entityPos.subtract(oldEntityPos);
    ...
    private static void handleVerticalCollision(Entity entity, AxisAlignedBB box, Vector3d motion){
        ...
        if(oldEntityBox.maxX > box.minX && oldEntityBox.minX < box.maxX && oldEntityBox.maxZ > box.minZ && oldEntityBox.minZ < box.maxZ){
            if(oldEntityBox.maxY < box.minY && entityBox.maxY > newBox.minY){
                ...
                PlayerEntity controllingPlayer = entity instanceof PlayerEntity ? (PlayerEntity)entity :
                    entity.getControllingPassenger() instanceof PlayerEntity ? (PlayerEntity)entity.getControllingPassenger() : null;

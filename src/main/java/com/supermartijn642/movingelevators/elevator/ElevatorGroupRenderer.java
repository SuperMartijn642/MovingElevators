        ...
        for(int x = 0; x < group.getCageSizeX(); x++){
            for(int y = 0; y < group.getCageSizeY(); y++){
                for(int z = 0; z < group.getCageSizeZ(); z++){
                    ...
                    List<BlockStateModelPart> parts = blockRenderState.setupModel(IDENTITY_MATRIX, model.hasMaterialFlag(ClientElevatorCage.getFakeLevel(), dummyBlockPos, block, BakedQuad.FLAG_TRANSLUCENT));
                    RANDOM_SOURCE.setSeed(block.getSeed(dummyBlockPos));
                    model.collectParts(ClientElevatorCage.getFakeLevel(), dummyBlockPos, block, RANDOM_SOURCE, parts);

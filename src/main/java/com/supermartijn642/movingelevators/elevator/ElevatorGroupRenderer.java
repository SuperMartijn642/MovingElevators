            ...
            for(int y = 0; y < group.getCageSizeY(); y++){
                for(int z = 0; z < group.getCageSizeZ(); z++){
                    ...
                    if(state.getRenderShape() == RenderShape.MODEL){
                        ...
                        renderType = renderType == RenderType.translucent() ? Sheets.translucentCullBlockSheet() : Sheets.cutoutBlockSheet();
                        blockRenderer.renderBatched(state, pos, level, poseStack, bufferSource.getBuffer(renderType), true, level.random);
                    }
                    poseStack.popPose();
                }
            }

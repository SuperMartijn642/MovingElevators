...
public class SliderWidget extends BaseWidget {
    ...
    public void render(int mouseX, int mouseY){
        ...
        // Background
        ScreenUtils.drawTexture(this.x, this.y, 1, this.height, 0, 0, 1 / 18f, 1);
        ScreenUtils.drawTexture(this.x + 1, this.y, this.width - 2, this.height, 1 / 18f, 0, 1 / 18f, 1);
        ScreenUtils.drawTexture(this.x + this.width - 1, this.y, 1, this.height, 2 / 18f, 0, 1 / 18f, 1);
        // Slider
        float percentage = (float)(this.value - this.min) / this.range;
        ScreenUtils.drawTexture(this.x + percentage * (this.width - 5), this.y, 5, this.height, this.active ? this.isFocused() || this.dragging ? 8 / 18f : 3 / 18f : 13 / 18f, 0, 5 / 18f, 1);
        // Text
        ITextComponent text = this.text.apply(this.value);
        if(text != null)
            ScreenUtils.drawCenteredStringWithShadow(text, this.x + this.width / 2f, this.y + 2, ScreenUtils.ACTIVE_TEXT_COLOR);
    }

...
public class DisplayBlockEntityRenderer implements CustomBlockEntityRenderer<DisplayBlockEntity> {
    ...
    private void drawOverlayPart(PoseStack poseStack, MultiBufferSource bufferSource, boolean transparent, int combinedLight, int combinedOverlay, Direction facing, float x, float y, float width, float height, int tX, int tY, int tWidth, int tHeight){
        ...
        buffer.vertex(matrix, x, y + height, 0).color(255, 255, 255, 255).uv(maxU, minV).overlayCoords(combinedOverlay).uv2(combinedLight).normal(poseStack.last(), facing.getStepX(), facing.getStepY(), facing.getStepZ()).endVertex();
        buffer.vertex(matrix, x + width, y + height, 0).color(255, 255, 255, 255).uv(minU, minV).overlayCoords(combinedOverlay).uv2(combinedLight).normal(poseStack.last(), facing.getStepX(), facing.getStepY(), facing.getStepZ()).endVertex();
        buffer.vertex(matrix, x + width, y, 0).color(255, 255, 255, 255).uv(minU, maxV).overlayCoords(combinedOverlay).uv2(combinedLight).normal(poseStack.last(), facing.getStepX(), facing.getStepY(), facing.getStepZ()).endVertex();
        buffer.vertex(matrix, x, y, 0).color(255, 255, 255, 255).uv(maxU, maxV).overlayCoords(combinedOverlay).uv2(combinedLight).normal(poseStack.last(), facing.getStepX(), facing.getStepY(), facing.getStepZ()).endVertex();
    }

...
public class DisplayBlockEntityRenderer implements CustomBlockEntityRenderer<DisplayBlockEntity> {
    ...
    public void render(DisplayBlockEntity entity, float partialTicks, int combinedOverlay, float alpha){
        ...
        for(int i = 0; i < total; i++){
            ...
            this.drawOverlayPart(false, combinedLight, combinedOverlay, facing, 0, 0, 1, 1, startIndex + i == index ? 96 : 64, labelColor.getMetadata() * 4, 32, 4);
    ...
    private void drawOverlayPart(boolean transparent, int combinedLight, int combinedOverlay, EnumFacing facing, float x, float y, float width, float height, int tX, int tY, int tWidth, int tHeight){
